...
import utils from '../AuthService/subscriptionsUtils';
...
class SubNewMovieComp extends PureComponent {
  ...
  componentDidMount = async () => {
    const resp = await utils.getUnWatchedMovies(this.props.data);
    if (resp)
  ...
  subscribe = async () => {
    const movie = {
      MemberId: this.props.data,
      Movie: {
        movieId: this.state.movieId,
        date: this.state.date
      }
    };
    let resp = await utils.subscribeMovie(movie);
    this.props.showMsgCallback(resp);
    if (resp) {
      resp = await utils.getUnWatchedMovies(this.props.data);
      if (resp)
