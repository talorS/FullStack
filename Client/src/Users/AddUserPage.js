...
class AddUserComp extends PureComponent {
  ...
  addUser = async (e) => {
    ...
    let obj = {
      ...
      Permissions: this.state.permissions.filter(obj => obj.checked).map(obj => obj.permission),
      Role : 'user'
    }
    let res = await utils.addUser(obj);
    this.setState({ created: res.status === 200? true : false });
  }
