...
class MainComp extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      name: authSrv.USER_AUTH.get().userInfo.name,
      role: authSrv.USER_AUTH.get().userInfo.role,
      btnsColor: this.props.data.btnsColor,
      timer: {
        minutes: authSrv.USER_AUTH.get().userInfo.timeout - 1,
        seconds: 60
      }
    }
  }
    ...
    return (
      <Container component="main" maxWidth="xs" item xs='auto'>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={3}>
            {this.state.role == "admin" && <Button size="small" variant="contained" className={classes.btn}
