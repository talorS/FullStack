...
class MemberComp extends PureComponent {
  ...
  componentDidMount = async () => {
    ...
    const resp = await utils.getMember(this.state.member.id);
    if (resp)
      this.setState({
        member: {
          ...this.state.member,
    ...
    else this.props.history.push('/MainPage/SubscriptionsPage/AllMembers');
  }

  deleteMember = async () => {
    const resp = await utils.deleteMember(this.state.member.id);
    if (resp.status === 200) {
