...
import utils from '../AuthService/membersUtils';
...
class AllMembersComp extends PureComponent {
  ...
  componentDidMount = async () => {
    const resp = await utils.getMembers();
  ...
  deleteMember = async (id, name) => {
    const resp = await utils.deleteMember(id);
    if (resp.status === 200) {
      let sliceMembers = [...this.state.members];
      let index = sliceMembers.map(member => member._id).indexOf(id);
      if (index !== -1) {
  ...
  render() {
    ...
    const membersList = this.state.members.map((member) => {
      return <div key={member._id} style={{ border: "2px solid black" }}>
        <ListItem>
        ...
        <Grid>
          <WatchedMoviesComp data={member._id} />
        </Grid>

        <Button size="small" variant="contained"
          disabled={this.state.userPermissions.indexOf('Update Subscriptions') == -1}
          onClick={() => this.props.history.push('/MainPage/SubscriptionsPage/AllMembers/EditMember/member/' + member._id, member)}>
          <EditOutlinedIcon />
          Edit
        </Button>
        <Button size="small" variant="contained"
          disabled={this.state.userPermissions.indexOf('Delete Subscriptions') == -1}
          onClick={() => this.deleteMember(member._id, member.Name)}>
